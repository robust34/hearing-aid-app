...
class CustomMessageActionsModal extends StatefulWidget {
  const CustomMessageActionsModal({
    super.key,
  ...
  final OnMessageTap? onEditTap;

  /// Callback when message is edited
  final OnMessageTap? onEdited;
  ...
  @override
  _CustomMessageActionsModalState createState() =>
      _CustomMessageActionsModalState();
}

class _CustomMessageActionsModalState extends State<CustomMessageActionsModal> {
  ...
  Widget _showMessageOptionsModal() {
    ...
    // Determine the positioin of modal
    int nearestEdgeJndex = 0;
    final middlePos = MediaQuery.of(context).size.height / 2;
    double contentPos = middlePos;
    if (widget.pos.isNotEmpty) {
      nearestEdgeJndex =
          (middlePos - widget.pos[0]).abs() < (widget.pos[1] - middlePos).abs()
              ? 0
              : 1;
      contentPos = nearestEdgeJndex > 0
          ? min(widget.pos[1] - 100, middlePos)
          : min(MediaQuery.of(context).size.height - widget.pos[0], middlePos);
    }
    ...
    final child = SingleChildScrollView(
      ...
      child: SizedBox(
        ...
        child: Column(
          // verticalDirection: nearestEdgeJndex > 0
          //     ? VerticalDirection.down
          //     : VerticalDirection.up,
          mainAxisAlignment: nearestEdgeJndex > 0
          ...
          children: <Widget>[
            if (nearestEdgeJndex > 0) SizedBox(height: contentPos),
            Padding(
              padding: EdgeInsets.only(
                left: widget.reverse ? 0 : 60,
              ),
              child: SizedBox(
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          if (widget.showWatchButton == true &&
                              _isMyMessage == false)
  ...
  Widget _buildReplyButton(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            SvgPicture.asset('assets/images/reply-icon.svg'),
            const SizedBox(width: 16),
            Text(
              '返信する',
              style: AppStyles.messageAction,
            ),
          ],
        ),
      ),
  ...
  Widget _buildCopyButton(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            SvgPicture.asset('assets/images/copy-icon.svg'),
            const SizedBox(width: 16),
            Text(
              'テキストをコピー',
              style: AppStyles.messageAction,
  ...
  Widget _buildEditMessage(BuildContext context) {
    ...
    return InkWell(
      onTap: () async {
        Navigator.pop(context);
        widget.onEditTap?.call(widget.message);
  ...
  void _showEditBottomSheet(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      widget.onEdited?.call(widget.message);
    });
  }
  ...
  Widget _buildWatchButton(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            SvgPicture.asset('assets/images/watch-icon.svg',
                color: const Color(0xffebebeb)),
            const SizedBox(width: 16),
            const Text(
              '通知を受け取る',
              style: TextStyle(
                  color: Color(0xffebebeb), fontWeight: FontWeight.w500),
            ),
          ],
        ),
      ),
